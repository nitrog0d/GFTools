...
public class WorldServerClient : ProxyClientBase {

    public override bool HandleClientPacket(ushort commandId, ref byte[] command) {
        Logger.InfoLine($"Client: {Enum.GetName((WorldServerCommands.CW_Commands)commandId)}");
    ...
    public override bool HandleServerPacket(ushort commandId, ref byte[] command) {
        Logger.InfoLine($"Server: {Enum.GetName((WorldServerCommands.CW_Commands)commandId)}");
